...
import (
	"GASE-SUPERLOGOS/controllers/services/payments"
...
func paymentsHandler(orderID int, gateway *models.Gateways, price float32, countries *models.Countries, paymentData map[string]interface{}, client *models.Clients) (paid bool, redirect string, paymentID string, err error) {
	...
	case "05": // safetyPay
		...
		data, err := payments.SafetyPayCreateExpressToken("EUR", price, orderID, "http://liderlogos.com/gracias?por=compra", "http://liderlogos.com/gracias?por=no-pago", filter, client)
...
func (c *GatewaysController) GetSafetypayTestRequestToken() {
	...
	client, err := models.GetClientsByID(1)
	...
	json, err := payments.SafetyPayCreateExpressToken("EUR", 300, 1, "http://liderlogos.com/gracias?por=compra", "http://liderlogos.com/gracias?por=no-pago", "online", client)
