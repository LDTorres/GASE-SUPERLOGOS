...
type SketchsFiles struct {
	...
	Sketch    *Sketchs  `orm:"column(skecths_id);rel(fk)" json:"skecths,omitempty"`
...
func (t *SketchsFiles) TableName() string {
	return "sketchs_files"
}

func (t *SketchsFiles) loadRelations() {
...
//GetSketchsFilesByID retrieves SketchsFiles by Id. Returns error if Id doesn't exist
func GetSketchsFilesByID(id int) (v *SketchsFiles, err error) {
	v = &SketchsFiles{ID: id}
...
func GetAllSketchsFiles(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(SketchsFiles))
	// query k=v
	for k, v := range query {
...
//UpdateSketchsFilesByID updates SketchsFiles by Id and returns error if the record to be updated doesn't exist
func UpdateSketchsFilesByID(m *SketchsFiles) (err error) {
	o := orm.NewOrm()
	v := Sketchs{ID: m.ID}
...
//DeleteSketchsFiles deletes SketchsFiles by Id and returns error if the record to be deleted doesn't exist
func DeleteSketchsFiles(id int, trash bool) (err error) {
	o := orm.NewOrm()
	v := SketchsFiles{ID: id}
	// ascertain id exists in the database
	err = o.Read(&v)
